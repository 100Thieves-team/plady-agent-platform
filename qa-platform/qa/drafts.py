...
DRAFT_SYSTEM = (
    ...
    "5. 쓰기 스크립트(POST/PUT/PATCH/DELETE)는 자기가 만든 데이터를 자기가 닫는 정리 단계(취소·철회·삭제)로 끝난다. 만든 데이터의 title 은 `[QA]` 로 시작한다. 룸 취소(`POST /v1/rooms/{roomId}/cancellation`)는 모집 중(RECRUITING)인 룸만 된다. 진행 확정 이후까지 간 룸(setup.room-confirmed 같은 카드를 쓴 경우 포함)은 취소가 E1410 으로 막히므로 `DELETE /v1/dev/rooms/{{roomId}}`(dev 전용 QA 룸 삭제, 제목이 [QA] 인 룸만)로 정리한다. 정리 단계에는 `always: true` 를 달아 앞 단계가 실패해도 돌게 한다.\n"
