...
### 8.2 정리 단계와 남은 데이터 (2026-09-25)

계기: `guestbook.post-happy` 가 진행 확정한 룸을 취소로 정리하려다 E1410 으로 실패해 룸이 dev 에 남았다. 다음 실행의 룸 생성은 새 룸 대신 그 확정 룸을 돌려줬다. 룸 생성은 (방장, 공고, 직무, 시작 시각)이 같은 활성 룸이 있으면 새로 만들지 않기 때문이다(백엔드 `RoomManager.findDuplicate`, 규칙표 `C.room.create` 멱등). 한 번 남은 데이터가 다음 실행을 계속 망쳤다.

- **`always: true`:** 단계에 달면 앞 단계가 실패해도 그 단계는 돈다. 정리 단계에 쓴다. 판정은 처음 실패한 단계가 정하고, 실패 뒤에 돈 정리 단계는 결과 화면에 "실패 뒤 정리" 로 보인다. 필요한 값(예: `roomId`)이 없으면 그 정리 단계는 건너뛴다(skipped). 폼에는 단계마다 "앞 단계가 실패해도 이 단계는 실행한다" 칸이 있다.
- **검증이 틀려도 값은 저장한다:** 응답에 `save` 경로의 값이 있으면 검증 결과와 상관없이 변수에 담는다. 그래야 정리 단계가 만든 데이터를 지울 수 있다. 실패 뒤에는 always 단계만 돌므로 다른 단계에는 영향이 없다.
- **`{{time:rand}}`:** 09:00~20:50 사이 10분 단위 임의 시각. 룸을 만드는 스크립트와 테스트 데이터 만들기 카드는 시작 시각을 이것으로 쓴다. 남은 룸과 시각이 겹쳐 기존 룸을 돌려받는 일을 줄인다.
- **정리는 가능하면 QA 룸 삭제로:** 모집 중이 아닌 룸은 취소가 안 된다(E1410). `DELETE /v1/dev/rooms/{{roomId}}`(dev 전용, 제목 [QA] 룸만)는 상태와 상관없이 지운다. 진행 확정 단계가 있는데 취소로 끝나는 스크립트는 저장할 때 경고한다.
- **Hermes:** 스크립트 생성 규칙에 위 셋(always, time:rand, 확정 룸 정리)을 적었다. 실패 분석에는 실패한 단계가 부른 API 에 묶인 규칙표 명령(수행자, 멱등 여부, 메모)과 게이트 검사를 준다. 응답이 규칙대로면 버그로 분류하지 말고 남은 데이터나 스크립트를 먼저 의심하라고 안내한다.
