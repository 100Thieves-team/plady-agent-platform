...
│   ├─ 5.5 초안함  [도메인 초안 생성](Hermes) → 목록 → 승인/반려 → YAML 다운로드
...
| P2 ✅ | TC 카탈로그(SSOT·OpenAPI·서술 파생)·`covers` 검증·커버리지·드리프트 배지·Hermes 초안 생성·초안함·탐색기(Swagger 모드)·위키 보고서 발행 버튼·가이드 화면 — 설계·구현 결과 [`qa-platform-tc.md`](qa-platform-tc.md) |
