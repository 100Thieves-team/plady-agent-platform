...
| P5 | **설계 중** — Swagger UI(`/swagger`, 기록 없이 그냥 호출해 보기 + op 별 QA 배지, 플랫폼이 CORS 전달·테스트 계정 토큰) · API 별로 TC·스크립트·최근 호출을 모아 보는 화면(`/apis`), API 호출 화면 마찰 줄이기(최근 값·즐겨찾기·프리필), 버튼 하나로 테스트 데이터 만들기(`/setup`, suite setup), 통과율·flaky 배지 — 설계 [`qa-platform-api.md`](qa-platform-api.md) (토스 QA Platform·Tossion 참고) |
