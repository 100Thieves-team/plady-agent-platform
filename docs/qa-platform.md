...
| P4 | QA MCP 도구 ✅(P4a, `qa/mcp_server.py` — Hermes 가 기준·케이스·런을 읽고 초안을 낸다) · Hermes 채팅창 ✅(P4b, `/chat`, `qa/chat.py`, Hermes `/v1/responses`) · 바뀐 TC 에 맞게 스크립트 다시 쓰기 ✅(P4c, `POST /cases/{id}/revise`, 초안 source hermes-revise + diff) · PRD 절에서 서술 TC 제안(P4d, 도구는 P4a 에 포함) — 설계 [`qa-platform-hermes.md`](qa-platform-hermes.md) |
