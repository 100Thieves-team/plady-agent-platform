...
| 화면 (지금) | 문서의 옛말 | 뜻 |
| --- | --- | --- |
| 테스트 케이스 (TC) | 기준 · 카탈로그 · TC | 무엇을 확인해야 하는가 — SSOT·PRD·OpenAPI 에서 파생 |
| 테스트 스크립트 | 케이스 | TC 를 확인하는 실행 단위(YAML, git 원본) |
| 검증하는 TC | 덮는 TC · covers | 스크립트가 확인한다고 선언한 TC |
| 정합성 (정합 · 경고 · 불일치) | 대조 (OK · 경고 · 오류) | 스크립트 선언과 TC 목록·API 계약의 일치 검사 |
| TC 변경 | 근거 변경 · 드리프트 | 검증하는 TC 가 마지막 검토 이후 바뀜 |
| 자동화됨 · 미자동화 · 자동화 제외 | 덮음 · 미커버 · 제외 | TC 를 검증하는 스크립트 유무 |
...
| 비즈니스 규칙 · API 계약 · 수동 작성 | 정책 · 계약 · 서술 | TC 의 세 층 |
...
| 이 API 에 해당하는 TC | 걸린 TC | API 계약 TC(operation) + API 매핑으로 이어진 비즈니스 규칙·수동 작성 TC |

## 0. 한 줄 요약

dev 서버에 배포된 백엔드를 **OpenAPI 계약과 PRD/SSOT 근거로 검증**하고, **누가 언제 무엇을 검증했는지 남기는** 얇은 플랫폼.
실행 버튼은 사람이 누르고, 실행 자체는 결정론 러너가 하고, AI(Hermes)는 **케이스 초안 생성과 실패 진단**만 맡는다.
...
| 토스 교훈 | 케이스 ↔ 런 분리, 런은 스냅샷 불변, Normal/Swagger 두 모드, AI 산출물은 검증 겹. | 그대로 채택. 실기기·시각 검증은 우리 범위 밖. |
...
4. **케이스의 정본은 git이다.** DB는 실행 이력과 초안만 갖는다.
5. **과거 런은 불변이다.** 실행 시점의 케이스 본문을 런에 박아 둔다.

## 3. 범위

**1차 범위(이 이슈)**
- HTTP API 레벨 QA — 대상 `dev`. 케이스 실행·기록·UI·사람 트리거 4종(배포 검증 / 스프린트 smoke / 릴리스 QA / 임의 선택).
- 감사 로그와 조회 UI.
- Swagger 기반 수동 QA(탐색기).
- Hermes 연동: 케이스 초안 생성(PRD+SSOT+OpenAPI 근거), 실패 진단.
...
- 케이스 편집기 UI(케이스는 git YAML이 정본, UI는 초안까지만).
...
│
├─ 1. 케이스 — 무엇을 검증하나            정본: git YAML (qa-platform/cases/)
...
│   ├─ 2.2 변경 범위  배포 SHA → PR → 변경 파일 → 도메인·operation → 권장 케이스 집합
│   ├─ 2.3 스프린트 상태  현재 Linear 사이클에 smoke 런이 있는가 → 없으면 배지
│   └─ 2.4 스펙 드리프트  현재 스펙의 operation 중 어느 케이스도 안 건드리는 것 목록(커버리지 공백)
│
...
│   ├─ 3.3 실행  케이스 순차, 단계 실패 시 그 케이스 중단, 런은 큐로 직렬화(dev 데이터 충돌 방지)
│   └─ 3.4 정리  케이스가 만든 데이터는 케이스가 닫는다(룸 생성 → 취소). 제목 접두 `[QA]`
│
├─ 4. 기록 — 증발하지 않게                sqlite, 볼륨 qa-data
│   ├─ 4.1 런  트리거·운영자·환경·SHA·PR·시각·판정 + 실행 시점 케이스 스냅샷(해시+본문)
│   ├─ 4.2 단계 결과  요청/응답(Authorization 마스킹, 8 KB 절단)·단언 결과·소요
│   ├─ 4.3 감사 로그  모든 사람 행위(런 생성·초안 승인·위키 발행·릴리스 판정·탐색기 전송)
│   ├─ 4.4 알림  Slack(`WIKI_SLACK_WEBHOOK_URL`) — 런 시작·완료 요약 + 실패 케이스 링크
│   └─ 4.5 위키 보고서  사람이 [위키에 발행]을 누를 때만 wiki/qa/ 에 generated 페이지
│
├─ 5. UI — 서버 렌더 HTML, 프레임워크 없음
│   ├─ 5.1 대시보드  미검증 배포 · 스프린트 상태 배지 · 최근 런 · 트리거 버튼 4종
│   ├─ 5.2 런 상세  케이스·단계별 요청/응답/단언, [Hermes 진단] → 분류(버그 / 케이스 노후 / 환경) + 근거
│   ├─ 5.3 케이스  목록·상세(근거 링크, 마지막 판정 이력), 커버리지 공백
│   ├─ 5.4 탐색기(Swagger 모드)  operation 선택 → 스펙 예제 프리필 → actor 선택 → 전송 → [케이스 단계로 저장]
│   ├─ 5.5 케이스 초안  [도메인 초안 생성](Hermes) → 목록 → 승인/반려 → YAML 다운로드
...
- ③ 케이스는 dev에만 쓴다. `[QA]` 접두·QA 전용 회원으로 사람 데이터와 분리한다.
...
대시보드
  ...
  확인 화면 — 실행 전에 무엇을 하는지 보여준다
    ...
    범위    sanity 4건 (변경 파일 → logging·member 도메인)  [전체 sanity로 넓히기] [케이스 직접 고르기]
...
| 행위 | 남는 것 |
| --- | --- |
| `run.create` | 운영자, 트리거 종류, 대상 SHA/PR, 선택된 케이스 목록과 선택 근거 |
...
이슈(MOI) → 구현(하네스: requirement-implementation, qa-reviewer 정적 리뷰) → PR → 리뷰·머지(dev)
   ...
   → 실패면: 런 상세에서 [Hermes 진단] → 버그면 Linear 이슈, 케이스 노후면 케이스 PR
```
...
- 픽스처(공고 id, 이력서 id)는 케이스가 아니라 **환경 설정**에 둔다 — dev 데이터가 바뀌면 설정만 고친다.
...
케이스 정본은 DB가 아니라 git이다. DB는 **실행 이력·감사 로그·초안**만 갖는다. 실행 시점의 케이스 본문을 런에 박아 두므로 케이스 YAML을 고쳐도 과거 런은 그대로다(토스 스냅샷 원칙).
...
| 케이스 = 이 레포의 git YAML | 리뷰 가능, 에이전트가 PR로 제안, 플랫폼 이미지와 함께 배포되어 백엔드 레포 의존이 없다 | 백엔드 레포 `qa/`(코드와 같은 PR에 실리는 장점, 대신 플랫폼이 남의 레포를 체크아웃해야 함), DB 편집기(정본 둘) |
...
| 결정론 러너, AI는 초안·진단만 | 매 실행에 LLM을 쓰면 비용·비결정성. 이슈 요구 "매번 모든 케이스를 계산하는 비용" 회피 | 에이전트가 매번 탐색 실행 |
...
- **기능별 도움말 "?"**: 화면의 제목·표 머리·버튼 옆 `?` 를 누르면 모달 — 무엇인지 · 언제 쓰는지 · 어떻게 하는지. 내용은 `qa/help.py` 의 `HELP` 표(70여 개), `ui.h("키")` 가 버튼, `/static/help.js` 가 모달. 테스트가 화면이 쓰는 키가 전부 표에 있는지 검사한다. 특히 "자동화됨 · 미자동화" 도움말이 미자동화 TC 를 자동화하는 길(초안 생성 → 한 번 실행 → 승인 → PR, 또는 exclusions)을 적는다.
