...
| P5 | **설계 중** — 토스식 호출 카드(Normal 폼 / Swagger 요청 원문 토글, API 호출·준비 작업 공용, op 별 QA 배지) · CSS 손질 · 실행 결과 요약(카드·도넛·도메인별 진행 막대) · API 별로 TC·스크립트·최근 호출을 모아 보는 화면(`/apis`), API 호출 화면 마찰 줄이기(최근 값·즐겨찾기·프리필), 버튼 하나로 테스트 데이터 만들기(`/setup`, suite setup), 통과율·flaky 배지 — 설계 [`qa-platform-api.md`](qa-platform-api.md) (토스 QA Platform·Tossion 참고) |
