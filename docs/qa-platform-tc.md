...
| **AI 산출물은 근거를 달고 검증 겹을 지난다** | 초안의 모든 케이스는 `covers` 가 있어야 하고, 플랫폼의 결정론 검증을 통과한 것만 초안함에 들어간다. 승인 없이 스위트에 들어가는 케이스는 없다(§7.3 유지) |
...
| **P2b 초안** | 근거 조립, Hermes 호출, 결정론 검증, 초안함, draft-check 런 | P2a 에 의존 |
| **P2c 탐색기** | op 폼, 전송=런, 단계로 담기 | P2a 의 스펙 파서에 의존. 초안함 재사용 |
