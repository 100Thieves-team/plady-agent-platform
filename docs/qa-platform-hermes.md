...
UI 안에 Hermes 채팅창을 둔다. Hermes 는 플랫폼이 내주는 **QA 도구(MCP)** 로 기준·스크립트·런·커버리지를 읽고, 스크립트를 새로 쓰거나 고쳐 **스크립트 초안**으로 낸다. 초안은 지금과 같은 결정론 검증과 사람 승인을 거쳐야 스크립트가 된다. **실행·전송·발행·승인 버튼은 여전히 사람만** 누른다. 같이 넣는 것: 기준이 바뀐 스크립트를 Hermes 가 새 기준에 맞게 다시 쓰는 버튼.
...
| **스크립트로 들어가는 문은 하나** | 채팅에서 만든 것도 스크립트 초안 → 결정론 검증 → 사람 승인 → PR. 문을 더 만들지 않는다 |
| **테스트 조건 정본은 Hermes 가 못 고친다** | SSOT·OpenAPI 는 읽기만. 서술 테스트 조건(`manual-tc.yaml`)도 "제안" 초안까지. 기준을 고치려면 위키를 고친다 |
| **근거는 남는다** | 버튼 경로 = 재현 가능한 프롬프트(해시). 채팅 경로 = 대화 기록 + 도구 호출 로그. 둘 다 감사 로그에 운영자와 함께 |
| **Hermes 가 하는 일은 읽기·제안·설명** | 커버리지 공백 설명, 실패 원인 해석, 스크립트 작성·수정 제안, 기준 변경 영향 설명 |
...
│   ├─ 2.2 컨텍스트 첨부  런 상세·스크립트 상세·TC 상세에 [Hermes 와 이야기] → 그 객체를 첫 메시지에 붙인다
│   ├─ 2.3 메시지 기록(sqlite) · 도구 호출 표시 · 초안이 만들어지면 링크
│   └─ 2.4 감사 로그  chat.create · chat.send · mcp.call
├─ 3. [바뀐 기준으로 초안 다시 쓰기]  스크립트 상세, 근거 변경 배지 옆
│   └─ 현재 YAML + 바뀐 TC 전/후 + 새 PRD 절 → Hermes → 검증 → 초안(원 스크립트 링크)
...
| 도구 | 인자 | 하는 일 | 쓰기 |
| --- | --- | --- | --- |
| `qa_catalog_search` | domain?, layer?, only?(uncovered\|covered\|warn), q?, limit | 테스트 조건 목록(제목·바인딩·덮는 스크립트·마지막 판정) | – |
| `qa_tc_get` | id | 레코드 전문 + PRD 절 본문 + 덮는 스크립트 | – |
| `qa_coverage` | – | 도메인×층 매트릭스, 제외 수, 기준 버전 | – |
| `qa_changes` | since? | 최근 바뀐 테스트 조건과 영향 스크립트 | – |
| `qa_case_list` / `qa_case_get` | domain?, suite? / id | 스크립트 목록 / YAML·covers·대조 상태·실행 이력 | – |
...
| `qa_draft_create` | yaml, reason | §7.2 검증 → 통과하면 스크립트 초안(source `hermes-chat`), 실패하면 사유를 돌려준다 | 초안만 |
| `qa_draft_update` | id, yaml | 결정되지 않은 초안만 재검증·갱신 | 초안만 |
| `qa_manual_tc_propose` | doc, section, items[] | 서술 테스트 조건 제안을 초안(kind `tc`)으로. 파일에는 사람이 옮긴다 | 초안만 |

없는 것(의도): 런 생성, 탐색기 전송, 위키 발행, 초안 승인·반려, 스크립트 파일 쓰기, 바인딩·제외 파일 쓰기.
...
- **첨부**: 실행·스크립트·테스트 조건 상세에서 패널을 열어 새 대화를 시작하면 "이 화면의 X 를 첨부" 체크박스가 켜져 있다. 상세의 [Hermes 와 이야기] 링크도 같은 일을 한다(`/chat/new?run=…`). 첨부는 첫 메시지 앞에 요약으로 붙고 Hermes 는 필요하면 도구로 나머지를 읽는다.
...
- 운영자 필수(쿠키). 없으면 패널이 먼저 운영자를 고르게 한다. 초안이 생기면 메시지 아래에 링크. 승인은 스크립트 초안 화면에서 사람이.
...
2. 스크립트 상세에서 근거 변경 배지가 있을 때 버튼이 뜬다. 플랫폼이 조립: 현재 스크립트 YAML · 바뀐 테스트 조건마다 전/후 레코드 · 사라진 테스트 조건 목록 · 새 PRD 절 본문 · 규칙("바뀐 부분만 고쳐라, covers 에서 사라진 id 는 빼고 대체 id 가 있으면 넣어라, 나머지 단계는 건드리지 마라").
3. Hermes 출력 → §7.2 검증(요청 테스트 조건 = 새 covers 후보 집합) → 초안(source `hermes-revise`, `case_id` 는 원 스크립트 id, note 에 어떤 테스트 조건이 어떻게 바뀌었는지).
4. 사람이 초안 화면에서 원본과 diff 를 보고 승인 → PR. 원 스크립트 파일을 플랫폼이 고치지 않는다.

버튼 경로이므로 프롬프트는 플랫폼이 조립하고 해시가 남는다(§13-8 유지).

### 3.4 Hermes 시스템 프롬프트(요지)

"너는 이 팀의 QA 엔지니어다. 기준(테스트 조건)은 SSOT·OpenAPI 에서 파생된 것이고 네가 만들지 않는다. 스크립트를 쓰거나 고칠 때는 `qa_draft_create` 로 초안을 내고, 검증 사유가 돌아오면 고쳐서 다시 낸다. 실행·발행·승인은 사람이 버튼으로 한다 — 요청받으면 어디서 누르는지 링크로 안내한다. 답은 도구로 읽은 사실에 근거하고, 모르는 것은 모른다고 한다. 회원 UUID 같은 식별값은 응답에 옮기지 않는다."

### 3.5 대표 시나리오

| 사람이 말하는 것 | Hermes 가 하는 것 |
| --- | --- |
| "room 도메인에서 아직 안 덮은 정책 테스트 조건 중 지금 스크립트로 만들 수 있는 것 골라 줘" | `qa_catalog_search(domain=room, layer=policy, only=uncovered)` → 제외 사유·바인딩 유무를 보고 후보 제시 |
| "G.room.create#8 스크립트 써 줘" | `qa_tc_get` · `qa_spec_op(createRoom)` · `qa_prd_section(룸 생성, 4.7)` → YAML → `qa_draft_create` → 검증 통과/사유 보고 + 초안 링크 |
| "어제 배포 검증 왜 실패했어?" | `qa_run_list(trigger=deploy-sanity)` · `qa_run_get(with_steps)` → 단계·응답 인용해 해석. 진단 버튼과 같은 분류 |
| "SSOT 가 바뀐 뒤 손봐야 할 스크립트 뭐야?" | `qa_changes` → 영향 스크립트와 무엇이 바뀌었는지 → [다시 쓰기] 버튼 안내 |
| "룸 탐색 PRD 4.2 절에서 테스트 조건 뽑아 줘" | `qa_prd_section` → `qa_manual_tc_propose` → 초안(kind tc) |
...
| P4d | [PRD 절에서 서술 테스트 조건 제안] | P4a 의 `qa_manual_tc_propose` 재사용 |
...
| 제안 도구의 저장 | `qa_draft_create` → drafts(source `hermes-chat`, operator `hermes`, note = reason). `qa_manual_tc_propose` → drafts **kind `tc`**(새 열, 기본 `case`) — 스크립트 초안 화면에 "서술 테스트 조건 제안" 배지, [한 번 실행해 보기] 없음, 승인 문구는 `manual-tc.yaml` 에 붙이라는 뜻. `qa_draft_update` 는 kind 에 맞는 검증을 다시 한다 | drafts.kind 는 §5 대로 |
...
| 검증 | 가짜 Hermes(SSE, 실제 `/mcp` 호출, 델타를 천천히)로 브라우저에서: 테스트 조건 화면에서 패널 열기 → 첨부 → 도구 칩이 진행 중에 뜸 → 본문 스트리밍 → 초안 링크 → 다른 화면으로 이동해도 같은 대화 → Enter 전송으로 2턴 체이닝 → `/chat/{id}` 인라인. 서버 오류 없음. 테스트 `tests/test_chat.py` 7건(전체 45건) | – |
...
### 10.3 P4c — 바뀐 테스트 조건에 맞게 스크립트 다시 쓰기 (2026-09-22)
...
| 버튼 | 스크립트 상세, 정합성 카드 아래, **테스트 조건 변경 배지가 있을 때만** [바뀐 테스트 조건에 맞게 Hermes 가 고치기 → 초안]. 담당자 필수, HERMES_API_KEY 없으면 비활성 | §3.3-2 대로 |
| 조립 | `drafts.assemble_revision()` — 현재 YAML · 바뀐 테스트 조건마다 변경 전/후 레코드 · 사라진 테스트 조건은 변경 전 + **대체 후보**(같은 변경 배치에서 추가됐고 id 앞부분이 같은 것, 없으면 지금 목록에서 앞부분 같은 것 6개까지) · 관련 OpenAPI 발췌 · 새 PRD 절 · **허용 covers**(현재 covers − 사라진 것 + 후보). 프롬프트 해시 저장 | 대체 후보 규칙은 구현에서 정함 |
| Hermes 규칙 | 바뀐 부분만, id 불변, 사라진 id 는 covers 에서 제거, 근거에 없는 값은 바꾸지 않음, 맨 위 `# 변경:` 주석 한 줄 (`REVISE_SYSTEM`) | §3.3-2 대로 |
| 검증·초안 | 출력의 id 를 원본 id 로 **강제**한 뒤 §7.2 검증(요청 테스트 조건 = 허용 covers, 기존 id 충돌 예외). 통과 → 초안 source `hermes-revise`, `case_id` = 원본, note 에 어떤 테스트 조건이 어떻게 바뀌었는지. 실패 → events `draft.rejected_by_validation` + 400 | §3.3-3 대로 |
...
**사람이 할 일**: 없음. 실제 테스트 조건 변경이 처음 생겼을 때(SSOT·OpenAPI 갱신 뒤) 스크립트 상세에서 버튼을 눌러 diff 가 말이 되는지 본다. `changes.json` 은 이 배포 이후의 변경부터 `before` 를 갖는다(이전 항목은 "(스냅샷 없음)" 으로 표시).

### 10.4 P4d — PRD 절에서 수동 작성 테스트 조건 제안 (2026-09-22)

| 항목 | 구현 | 설계 대비 |
| --- | --- | --- |
| 버튼 | 테스트 조건 화면 아래 카드: PRD 문서 이름 · 절 번호 · 도메인(선택) → [제안 받기]. 담당자·HERMES_API_KEY 필요 | 설계는 "서술 층 화면" 이었으나 층 구분 없이 같은 화면 하단에 |
| 조립 | 플랫폼이 PRD 절 본문(위키 체크아웃, 150줄) + **이미 그 절에서 뽑힌 수동 테스트 조건 목록**(중복 방지)을 넣는다. 프롬프트 해시 저장. `drafts.propose_manual_tc()` | 같음 |
| Hermes 규칙 | `items:` 목록(title·given·when·then·operations?), 본문에 적힌 것만, 최대 8개 (`PROPOSE_SYSTEM`) | – |
| 레코드 | MCP 도구 `qa_manual_tc_propose` 와 **같은 조립 함수** `drafts.build_manual_tc()` — id 는 `PRD.<slug>.<절>#n` 기존 다음 번호, 도메인은 인자 → 같은 문서의 기존 수동 테스트 조건 → other. 문서·절이 위키에 없으면 경고 | 도구와 버튼이 같은 문 |
...
- 구현 순서와 커밋: P4a QA MCP 서버 → P4b 채팅 위젯(모달·SSE) → 용어 정리 → P4c 다시 쓰기 → P4d 수동 테스트 조건 제안.
- Hermes 가 하는 일은 **읽기·제안·설명** 에 머문다. 실행·발행·승인·파일 쓰기 도구는 서버에 없다(원칙 ①). 위키 쓰기(`wiki_apply`)는 채팅 경로에서 가능하나 시스템 프롬프트로 금하고 git 으로 되돌린다(§8-1).
- 근거: 버튼 경로(초안 생성·다시 쓰기·수동 테스트 조건 제안)는 프롬프트 해시, 채팅 경로는 대화 기록 + `mcp.call` 로그.
