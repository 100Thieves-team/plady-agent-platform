...
- **n8n은 켜지 않는다.** PLA-251 계약이 live n8n 런타임을 명시적으로 금지하므로(compose 블록 주석, ALB 503 유지), n8n 기반 자동 수집은 PLA-251 활성화 체크리스트([`n8n-placeholder.md`](n8n-placeholder.md)) 완료 이후로 미룬다.
...
3. **webex**: 녹취/회의록 export API 조사. 그 전까지는 "webex 회의록을 notion에 남기는" 운영 규약으로 우회.
